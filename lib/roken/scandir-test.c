...
{
    return strcmp(entry->d_name, "alpha") == 0 ||
           strcmp(entry->d_name, "zeta") == 0;
}
...
main(void)
{
    const char *const names[] = { "zeta", "skip", "alpha" };
    ...
    if (nentries != 2 ||
        strcmp(entries[0]->d_name, "alpha") != 0 ||
        strcmp(entries[1]->d_name, "zeta") != 0) {
